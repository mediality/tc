...
assert.match(app, /expert: \[2, 4\]/);
assert.match(app, /champion: \[1, 3\]/);
assert.match(app, /legend: \[1, 1\]/);
assert.match(app, /amateur: 0\.18,[\s\S]*?normal: 0\.35,[\s\S]*?expert: 0\.68,[\s\S]*?champion: 0\.82,[\s\S]*?legend: 1/);
...
assert.match(app, /amateur: 0\.08,[\s\S]*?normal: 0\.2,[\s\S]*?expert: 0\.82,[\s\S]*?champion: 1,[\s\S]*?legend: 1/);
...
assert.match(css, /\.card-local-preview\.visible\s*\{\s*opacity: 1;/);
