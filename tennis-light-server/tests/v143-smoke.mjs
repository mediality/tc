...
async function register(email, nickname) {
  const session = { cookie: "", user: null, email, password: "Test-v143!" };
  const { response, data } = await request(session, "/api/auth/register", {
    method: "POST",
    body: JSON.stringify({ email, password: "Test-v143!", nickname }),
  });
...
const players = await Promise.all([
  register(`v143-a-${stamp}@example.test`, "Alpha"),
  register(`v143-b-${stamp}@example.test`, "Bravo"),
  register(`v143-c-${stamp}@example.test`, "Charlie"),
  register(`v143-d-${stamp}@example.test`, "Delta"),
]);
...
if (process.env.KEEP_AI_REVEAL === "1") {
  console.log(`v143 AI reveal: ${base}/?friendlyTournament=${progressReporter.access.id}&participant=${progressReporter.access.participantId}&token=${progressReporter.access.token}`);
  console.log("v143 AI reveal browser fixture: READY");
  process.exit(0);
}
...
if (process.env.KEEP_SHARED_DUEL === "1") {
  ...
  console.log(`v143 shared player A: ${participantUrl(playerAContext.access)}`);
  console.log(`v143 shared player B: ${participantUrl(playerBContext.access)}`);
  console.log(`v143 shared spectator: ${spectatorAccess.data.spectatorUrl}`);
  console.log("v143 shared browser fixture: READY");
  process.exit(0);
}
...
if (process.env.KEEP_RECONNECT_COUNTDOWN === "1") {
  const participantUrl = (access) => `${base}/?friendlyTournament=${access.id}&participant=${access.participantId}&token=${access.token}`;
  console.log(`v143 reconnect opponent: ${participantUrl(forfeitWinner.access)}`);
  console.log(`v143 reconnect player email: ${forfeitLoser.player.email}`);
  console.log(`v143 reconnect player password: ${forfeitLoser.player.password}`);
  console.log(`v143 reconnect tournament: ${forfeitLoser.access.id}`);
  console.log("v143 reconnect browser fixture: READY");
  process.exit(0);
}
...
if (process.env.KEEP_FORFEIT_TRANSITION === "1") {
  console.log(`v143 forfeit winner: ${base}/?friendlyTournament=${forfeitWinner.access.id}&participant=${forfeitWinner.access.participantId}&token=${forfeitWinner.access.token}`);
  console.log("v143 forfeit browser fixture: READY");
  process.exit(0);
}
...
console.log("v143 smoke test: OK");
