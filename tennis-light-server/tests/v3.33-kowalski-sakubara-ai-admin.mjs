...
assert.match(app, /const REMISE_UNDERLAY_IMAGE = "assets\/fond-carte-remise\.jpg\?v=3\.45"/);
assert.match(app, /const placementBonus = remiseCards\.reduce\([\s\S]*<span>\+\$\{placementBonus\}<\/span>/);
assert.match(css, /\.remise-underlay-wrap \{[\s\S]*width: 148px[\s\S]*height: 220px/);
...
console.log("v3.45 : nouveaux joueurs, profil, news, IA, RankIA, admin et finale multi-écrans : OK");
