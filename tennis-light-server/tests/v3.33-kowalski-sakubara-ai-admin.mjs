...
assert.match(app, /const REMISE_UNDERLAY_IMAGE = "assets\/fond-carte-remise\.jpg\?v=3\.44"/);
assert.match(app, /const placementBonus = remiseCards\.reduce\([\s\S]*<span>\+\$\{placementBonus\}<\/span>/);
assert.match(css, /\.remise-underlay-wrap \{[\s\S]*width: 148px[\s\S]*height: 254px/);
assert.match(css, /\.remise-underlay-layer \{[\s\S]*top: 20%/);
...
console.log("v3.44 : nouveaux joueurs, profil, news, IA, RankIA, admin et finale multi-écrans : OK");
