...
assert.match(app, /function playSelectedMobileCard\(\)/);
...
assert.match(mobileApp, /Jouer \(-\$\{preview\.realCost\} endurance\)/);
...
assert.match(mobileStyles, /\.mobile-hand-card--selected\s*\{[\s\S]*translateY\(-8px\) scale\(1\.045\)/);
