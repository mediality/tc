...
assert.match(server, /simulationIndex < 2/);
assert.doesNotMatch(server, /multiplier = rankIa/);
assert.match(server, /const rankCaps = \[1, 0\.85, 0\.8, 0\.75\]/);
assert.match(server, /const ratio = rankCaps\[index\] \?\? 0\.7/);
assert.match(server, /Math\.ceil\(rawPerformancePoints \/ 2\)/);
