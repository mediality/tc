...
const CIRCUIT_AI_CHARACTER_IDS = [...HISTORIC_CHARACTER_IDS, ...NEW_CHARACTER_IDS, ...NEXT_GEN_CHARACTER_IDS];
...
const POINT_TABLES = {
  400: { round32: 0, round16: 10, qualif: 10, quarter: 50, semi: 100, finalist: 200, winner: 400 },
  600: { round32: 5, round16: 20, qualif: 20, quarter: 75, semi: 150, finalist: 300, winner: 600 },
...
const authMemory = {
  ...
  circuitAiSimulationReports: new Map(),
...
function aiMatchStrength(characterId, competition, season, week, slot, bonusTopIds = [], simulationNonce = "", rankingOrder = [], seedBonusCounts = {}, motivationById = {}) {
  ...
  return 50 + intelligenceBonus + surfaceBonus + protectedBonus + tournamentBonus + motivationBonus + matchForm;
}
...
function aiMotivationStateKey(season, week) {
  return `ai_motivation_${season}_${week}`;
}

async function storedAiMotivation(season, week) {
  const stored = await getAppStateValue(aiMotivationStateKey(season, week), null);
  if (!stored) return null;
  try { return JSON.parse(stored); } catch (error) { return null; }
}

async function buildAiMotivationForWeek(season, week, standings, simulationNonce) {
  const existing = await storedAiMotivation(season, week);
  if (existing?.values) return existing;
  const previousPeriods = previousCircuitPeriods(season, week, 4);
  const history = [];
  for (const period of previousPeriods) {
    history.push(await storedAiMotivation(period.season, period.week));
  }
  const streak = (characterId, polarity) => {
    let count = 0;
    for (const record of history) {
      const value = Number(record?.values?.[characterId] || 0);
      if ((polarity > 0 && value > 0) || (polarity < 0 && value < 0)) count += 1;
      else break;
    }
    return count;
  };
  const previousParticipants = [...(standings.rows || [])]
    .filter((entry) => Number(entry.scorePreviousWeek || 0) > 0);
  const best = previousParticipants
    .filter((entry) => streak(entry.characterId, 1) < 4)
    .sort((a, b) => b.scorePreviousWeek - a.scorePreviousWeek || aiCharacterName(a.characterId).localeCompare(aiCharacterName(b.characterId), "fr"))[0]?.characterId || null;
  const worst = previousParticipants
    .filter((entry) => streak(entry.characterId, -1) < 4)
    .sort((a, b) => a.scorePreviousWeek - b.scorePreviousWeek || aiCharacterName(a.characterId).localeCompare(aiCharacterName(b.characterId), "fr"))[0]?.characterId || null;
  const positivePool = CIRCUIT_AI_CHARACTER_IDS.filter((id) => streak(id, 1) < 4);
  const randomPositive = positivePool[Math.floor(seededRandom(`${simulationNonce}:${season}:${week}:motivation:positive`) * positivePool.length)] || null;
  const positiveIds = new Set([best, randomPositive].filter(Boolean));
  const negativePool = CIRCUIT_AI_CHARACTER_IDS.filter((id) => !positiveIds.has(id) && streak(id, -1) < 4);
  const randomNegative = negativePool[Math.floor(seededRandom(`${simulationNonce}:${season}:${week}:motivation:negative`) * negativePool.length)] || null;
  const values = {};
  const add = (id, value) => { if (id) values[id] = Number(values[id] || 0) + value; };
  add(best, 1);
  add(randomPositive, 2);
  add(worst, -0.5);
  add(randomNegative, -2);
  const record = {
    season, week, values,
    assignments: { previousBest: best, randomPositive, previousWorst: worst, randomNegative },
    generatedAt: new Date().toISOString(),
  };
  await setAppStateValue(aiMotivationStateKey(season, week), JSON.stringify(record));
  return record;
}
...
function simulatedAiTournamentPoints(competition, season, week, bonusTopIds = [], simulationNonce = "", rankingOrder = [], motivationById = {}) {
  ...
  const ranked = rankingOrder.length === CIRCUIT_AI_CHARACTER_IDS.length
    ? [...rankingOrder]
    : [...CIRCUIT_AI_CHARACTER_IDS].sort(byRanking);
  const seeds = ranked.slice(0, 8);
  const positions = Array(32).fill(null);
  const seedSlots = [0, 31, 16, 15, 8, 23, 24, 7];
  seeds.forEach((characterId, index) => { positions[seedSlots[index]] = characterId; });
  const groupTwo = deterministicShuffle(ranked.slice(8, 16), `${simulationNonce}:${competition.id}:${season}:${week}:group2`);
  const groupThree = deterministicShuffle(ranked.slice(16, 24), `${simulationNonce}:${competition.id}:${season}:${week}:group3`);
  const groupFour = deterministicShuffle(ranked.slice(24, 32), `${simulationNonce}:${competition.id}:${season}:${week}:group4`);
  const remainingSlots = positions.map((value, index) => value ? null : index).filter((value) => value !== null);
  deterministicShuffle([...groupTwo, ...groupThree, ...groupFour], `${simulationNonce}:${competition.id}:${season}:${week}:remaining`)
    .forEach((characterId, index) => { positions[remainingSlots[index]] = characterId; });
  ...
  const playRound = (players, roundLabel, loserPoints) => {
    ...
    for (let index = 0; index < players.length; index += 2) {
      ...
      const strengthA = aiMatchStrength(playerA, competition, season, week, `${roundLabel}:${index}`, bonusTopIds, simulationNonce, rankingOrder, seedBonusCounts, motivationById);
      const strengthB = aiMatchStrength(playerB, competition, season, week, `${roundLabel}:${index + 1}`, bonusTopIds, simulationNonce, rankingOrder, seedBonusCounts, motivationById);
  ...
  const round32 = positions;
  const round16 = playRound(round32, "round32", table.round32 || 0);
  const quarterFinalists = playRound(round16, "round16", table.round16 ?? table.qualif ?? 0);
  ...
  return { tournamentPoints: awards, performancePoints: performanceAwards };
}

function simulatedAiLeaguePoints(competition, season, week, bonusTopIds = [], simulationNonce = "", rankingOrder = [], motivationById = {}) {
  const table = competition.points || POINT_TABLES[competition.value] || POINT_TABLES[800];
  const tournamentPoints = new Map(CIRCUIT_AI_CHARACTER_IDS.map((id) => [id, 0]));
  const performancePoints = new Map(CIRCUIT_AI_CHARACTER_IDS.map((id) => [id, 0]));
  const rankBands = [[1, 8], [9, 16], [17, 24], [25, 32]];
  const ranked = rankingOrder.length ? rankingOrder : CIRCUIT_AI_CHARACTER_IDS;
  const addTournament = (id, points) => tournamentPoints.set(id, (tournamentPoints.get(id) || 0) + Math.max(0, Number(points || 0)));
  const addPerformance = (id, points) => performancePoints.set(id, (performancePoints.get(id) || 0) + Math.max(0, Number(points || 0)));
  const playMatch = (playerA, playerB, label) => {
    const strengthA = aiMatchStrength(playerA, competition, season, week, `${label}:a`, bonusTopIds, simulationNonce, rankingOrder, {}, motivationById);
    const strengthB = aiMatchStrength(playerB, competition, season, week, `${label}:b`, bonusTopIds, simulationNonce, rankingOrder, {}, motivationById);
    ...
    performance.forEach((points, id) => addPerformance(id, points));
    addTournament(winner, table.matchWin || 0);
    return winner;
  };
  rankBands.forEach(([from, to], bandIndex) => {
    ...
    for (const groupName of ["A", "B"]) {
      ...
      addTournament(ordered[2], table.group3 || 0);
      addTournament(ordered[3], table.group4 || 0);
    }
    ...
    addTournament(semiLoser1, table.semi || 0);
    addTournament(semiLoser2, table.semi || 0);
    ...
    addTournament(finalist, table.finalist || 0);
    addTournament(champion, table.winner || 0);
  });
  return { tournamentPoints, performancePoints };
}
...
function maxWeeklyTournamentPoints(week) {
  return COMPETITION_DEFINITIONS
    ...
    .reduce((sum, competition) => {
      const table = competition.points || POINT_TABLES[competition.value] || POINT_TABLES[400];
      const leagueWins = competition.eventType === "League" ? Number(table.matchWin || 0) * 5 : 0;
      return sum + Number(table.winner || 0) + leagueWins;
    }, 0);
}

function maxWeeklyPerformancePoints(week) {
  return COMPETITION_DEFINITIONS
    .filter((competition) => competition.week === week)
    .reduce((sum, competition) => {
      const perMatch = Number(competition.targetSets || 2) * 11 + 5;
      return sum + perMatch * 5;
    }, 0);
}

function maxWeeklyAiPoints(week) {
  return maxWeeklyTournamentPoints(week) + maxWeeklyPerformancePoints(week);
}
...
async function clampAiWeekScoresToPointMax(season, week) {
  ...
  const pointMax = maxWeeklyAiPoints(safeWeek);
  if (db) {
    await db.query(`
...
async function aiCircuitStandingsForBoost(season, week) {
  ...
  if (db) {
    ...
    rows = CIRCUIT_AI_CHARACTER_IDS.map((characterId) => {
      ...
      return {
        characterId,
        scoreRef: Number(stored?.score_ref || 0) + Number(NEXT_GEN_CIRCUIT_POINTS[characterId] || 0),
        scorePreviousWeek: Number(stored?.score_previous_week || 0),
        scoreTotal: Number(stored?.score_total || 0) + Number(NEXT_GEN_CIRCUIT_POINTS[characterId] || 0),
      };
    });
  } else {
    rows = CIRCUIT_AI_CHARACTER_IDS.map((characterId) => ({
      characterId,
      scoreRef: sumMemoryAiPeriodScores(authMemory.circuitAiWeekScores, characterId, refPeriods) + Number(NEXT_GEN_CIRCUIT_POINTS[characterId] || 0),
      scorePreviousWeek: previousPeriodKey
        ? memoryAiWeekScore(characterId, ...previousPeriodKey.split(":").map(Number))
        : 0,
      scoreTotal: Number(NEXT_GEN_CIRCUIT_POINTS[characterId] || 0) + Array.from({ length: CIRCUIT_SEASON_LENGTH }, (_, index) => index + 1)
...
async function storeAiSimulationReport(season, week, report) {
  if (db) {
    await db.query(`
      INSERT INTO circuit_ai_simulation_reports (season_number, week_number, report_json, updated_at)
      VALUES ($1, $2, $3::jsonb, NOW())
      ON CONFLICT (season_number, week_number) DO UPDATE
        SET report_json = EXCLUDED.report_json, updated_at = NOW()
    `, [season, week, JSON.stringify(report)]);
  } else {
    authMemory.circuitAiSimulationReports.set(`${season}:${week}`, report);
  }
}

async function readAiSimulationReport(season, week) {
  if (db) {
    const result = await db.query(
      "SELECT report_json, updated_at FROM circuit_ai_simulation_reports WHERE season_number = $1 AND week_number = $2",
      [season, week],
    );
    return result.rows[0] ? { ...result.rows[0].report_json, updatedAt: result.rows[0].updated_at } : null;
  }
  return authMemory.circuitAiSimulationReports.get(`${season}:${week}`) || null;
}
...
async function simulateAiCircuitWeek(season, week, options = {}) {
  ...
  for (let simulationIndex = 0; simulationIndex < 2; simulationIndex += 1) {
    ...
    runs.push(Object.fromEntries(CIRCUIT_AI_CHARACTER_IDS.map((characterId) => [characterId, {
      tournamentPoints: runTournamentTotals.get(characterId) || 0,
      performancePoints: runPerformanceTotals.get(characterId) || 0,
    }])));
  }
  ...
  if (db) {
    for (const { characterId, points } of adjustedEntries) {
      await db.query(`
    ...
    await clampAiWeekScoresToPointMax(season, week);
    await storeAiSimulationReport(season, week, report);
    return;
  }
...
async function backfillCurrentLeagueAiPoints(current) {
  ...
  const combinedAwards = new Map(CIRCUIT_AI_CHARACTER_IDS.map((characterId) => [
    characterId,
    Number(awards.tournamentPoints.get(characterId) || 0) + Math.ceil(Number(awards.performancePoints.get(characterId) || 0) / 2),
  ]));
  if (db) {
    for (const [characterId, points] of combinedAwards) {
  ...
  return {
    ...
    aiCount: [...combinedAwards.values()].filter((points) => Number(points || 0) > 0).length,
  };
}
...
async function initAuthStorage() {
  ...
  `);
  await db.query(`
    CREATE TABLE IF NOT EXISTS circuit_ai_simulation_reports (
      season_number INTEGER NOT NULL,
      week_number INTEGER NOT NULL,
      report_json JSONB NOT NULL,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      PRIMARY KEY (season_number, week_number)
    )
  `);
  await db.query(`
...
async function buildRanking(page = 1, pageSize = 50, currentUser = null, sortBy = "points") {
    ...
    const rows = [
      ...
      ...CIRCUIT_AI_CHARACTER_IDS.map((characterId) => {
        ...
        return {
          ...
          score_ref: Number(scores.score_ref || 0) + Number(NEXT_GEN_CIRCUIT_POINTS[characterId] || 0),
          score_week: Number(scores.score_week || 0),
          score_next_ref: Number(scores.score_next_ref || 0) + Number(NEXT_GEN_CIRCUIT_POINTS[characterId] || 0),
          score_previous_week: Number(scores.score_previous_week || 0),
          score_total: Number(scores.score_total || 0) + Number(NEXT_GEN_CIRCUIT_POINTS[characterId] || 0),
          is_ai: true,
        };
      }),
    ];
  ...
  const aiRows = CIRCUIT_AI_CHARACTER_IDS.map((characterId) => {
    const initialPoints = Number(NEXT_GEN_CIRCUIT_POINTS[characterId] || 0);
    const scoreRef = initialPoints + sumMemoryAiPeriodScores(authMemory.circuitAiWeekScores, characterId, refPeriods);
    const scoreWeek = memoryAiWeekScore(characterId, current.season, current.week);
    const scoreNextRef = initialPoints + sumMemoryAiPeriodScores(authMemory.circuitAiWeekScores, characterId, nextRefPeriods);
    ...
    const scoreTotal = initialPoints + Array.from({ length: CIRCUIT_SEASON_LENGTH }, (_, index) => index + 1)
...
async function profilePayload(user, viewer = user) {
  ...
  return {
    ...
    activity: publicProfileActivity(user.id, viewer),
...
async function handleAuth(req, res, url) {
  ...
  if (req.method === "GET" && url.pathname === "/api/admin/ai-simulation-report") {
    if (!await requireAdmin(req, res)) return true;
    const current = await circuitState();
    const requestedSeason = Math.max(1, Number(url.searchParams.get("season") || current.season));
    const requestedWeek = Math.max(1, Math.min(CIRCUIT_SEASON_LENGTH, Number(url.searchParams.get("week") || current.week)));
    sendJson(res, 200, {
      season: requestedSeason,
      week: requestedWeek,
      report: await readAiSimulationReport(requestedSeason, requestedWeek),
    });
    return true;
  }
  ...
  if (req.method === "GET" && publicProfileWatchMatch) {
    ...
    const friendlyTournament = activity?.tournamentId ? friendlyTournaments.get(activity.tournamentId) : null;
    const canWatchPrivateEvent = !friendlyTournament || friendlyUserCanSpectate(friendlyTournament, viewer);
    if (!target || !activity || !activity.watchable || !activity.state || !canWatchPrivateEvent) {
...
function publicProfileActivity(userId, viewer = null) {
  const activity = activeProfileActivity(userId);
  if (!activity) return null;
  const friendlyTournament = activity.tournamentId ? friendlyTournaments.get(activity.tournamentId) : null;
  const canWatch = Boolean(
    activity.watchable
    && (!friendlyTournament || friendlyUserCanSpectate(friendlyTournament, viewer))
  );
  return {
    type: activity.type,
    opponent: activity.opponent,
...
function friendlyMasterControl(tournament) {
  ...
  return tournament.masterControl;
}

function friendlyParticipantStillQualified(tournament, participant) {
  if (!participant || participant.forfeitedAt || participant.eliminated) return false;
  const entry = friendlyParticipantEntry(participant.id);
  if (tournament.status === "complete") return tournament.champion === entry;
  if (tournament.format === "onepointmaster" && /^group[1-5]$/.test(tournament.round)) return true;
  if ((tournament.matches || []).some((match) => !match.winner && (match.playerA === entry || match.playerB === entry))) return true;
  const completed = (tournament.matches || []).filter((match) => match.winner && (match.playerA === entry || match.playerB === entry));
  return !completed.length || completed.at(-1).winner === entry;
}
...
function friendlyParticipantCanControl(tournament, participant) {
  return Boolean(participant && friendlyControlParticipantIds(tournament).has(participant.id));
}
...
function friendlyTournamentVisibility(tournament) {
  return tournament?.visibility === "private" ? "private" : "public";
}

function friendlyUserIsValidated(tournament, user) {
  if (!tournament || !user?.id) return false;
...
function friendlyUserCanSpectate(tournament, user) {
  return friendlyTournamentVisibility(tournament) === "public" || friendlyUserIsValidated(tournament, user);
}
...
function publicFriendlyTournamentInfo(req, tournament, participant = null, spectator = null) {
  ...
  return {
    ...
    seedNumbers: ["ranking", "separated"].includes(tournament.distribution)
    ...
    participants: activeParticipants.map((item) => ({
      ...
      eliminated: Boolean(item.eliminated || (tournament.status === "playing" && !friendlyParticipantStillQualified(tournament, item))),
      away: Boolean(item.awayAt),
      selected: Boolean(item.selected),
      forfeited: Boolean(item.forfeitedAt),
    ...
    groups: {
      A: hideMasterInitialDraw ? [] : (tournament.groups?.A || []).map((entry) => friendlyEntryPublic(tournament, entry)),
      B: hideMasterInitialDraw ? [] : (tournament.groups?.B || []).map((entry) => friendlyEntryPublic(tournament, entry)),
      C: hideMasterInitialDraw ? [] : (tournament.groups?.C || []).map((entry) => friendlyEntryPublic(tournament, entry)),
      D: hideMasterInitialDraw ? [] : (tournament.groups?.D || []).map((entry) => friendlyEntryPublic(tournament, entry)),
    },
    standings: tournament.format === "onepointmaster"
      ? Object.fromEntries(["A", "B", "C", "D"].map((group) => [group, hideMasterInitialDraw ? [] : friendlyOnePointMasterStandings(tournament, group).map((row, index) => ({
        ...row,
        position: index + 1,
    ...
    competitionControl: friendlyOnePointFormat(tournament) ? {
      ...
      currentRoundComplete: (tournament.matches || [])
        .filter((match) => match.round === tournament.round && match.playerA && match.playerB)
...
function publicFriendlyLobbyInfo(req, tournament, user = null) {
  ...
  return {
    ...
    visibility: friendlyTournamentVisibility(tournament),
    selectionLimit: friendlySelectionLimit(tournament),
    canResume: Boolean(currentParticipant && !currentParticipant.forfeitedAt && ["waiting", "playing"].includes(tournament.status)),
    canSpectate: Boolean(tournament.status === "playing" && friendlyUserCanSpectate(tournament, user)),
...
function separatedOnePointMasterGroups(rankedEntries, humanEntries) {
  const groupNames = ["A", "B", "C", "D"];
  const groups = Object.fromEntries(groupNames.map((group) => [group, Array(6).fill(null)]));
  const humans = new Set(humanEntries);
  const excludedGroups = new Set();
  const placed = new Set();
  const availableGroup = (slot) => shuffleFriendlyEntries(groupNames.filter((group) => (
    !excludedGroups.has(group) && !groups[group][slot]
  )))[0] || null;
  for (const entry of rankedEntries.filter((candidate) => humans.has(candidate))) {
    const rank = rankedEntries.indexOf(entry) + 1;
    const preferredSlot = rank <= 4 ? 0 : rank <= 8 ? 1 : null;
    let group = preferredSlot == null ? null : availableGroup(preferredSlot);
    let slot = preferredSlot;
    if (!group) {
      group = shuffleFriendlyEntries(groupNames.filter((candidate) => !excludedGroups.has(candidate)))[0] || null;
      slot = group ? groups[group].findIndex((value, index) => index >= 2 && !value) : -1;
    }
    if (!group || slot < 0) continue;
    groups[group][slot] = entry;
    placed.add(entry);
    excludedGroups.add(group);
  }
  const fillPot = (entries, slot) => {
    for (const entry of entries.filter((candidate) => !placed.has(candidate))) {
      const group = shuffleFriendlyEntries(groupNames.filter((candidate) => !groups[candidate][slot]))[0];
      if (!group) break;
      groups[group][slot] = entry;
      placed.add(entry);
    }
  };
  fillPot(rankedEntries.slice(0, 4), 0);
  fillPot(rankedEntries.slice(4, 8), 1);
  for (const entry of rankedEntries.filter((candidate) => !placed.has(candidate))) {
    const group = groupNames.find((candidate) => groups[candidate].some((value, index) => index >= 2 && !value));
    if (!group) break;
    const slot = groups[group].findIndex((value, index) => index >= 2 && !value);
    groups[group][slot] = entry;
    placed.add(entry);
  }
  return Object.fromEntries(groupNames.map((group) => [group, groups[group].filter(Boolean)]));
}
...
async function buildFriendlyTournamentBracket(tournament) {
  ...
  if (tournament.format === "onepointmaster") {
    ...
    if (tournament.distribution === "separated") {
      Object.assign(groups, separatedOnePointMasterGroups(rankedEntries, humanEntries));
    } else if (tournament.distribution === "ranking") {
...
function friendlyEntryHasLeft(tournament, entry) {
  ...
  const participant = tournament.participants.find((item) => friendlyParticipantEntry(item.id) === entry);
  return Boolean(participant?.leftAt || participant?.forfeitedAt);
}
...
function markFriendlyParticipantPresent(participant, now = Date.now()) {
  if (!participant || participant.forfeitedAt) return false;
...
function markFriendlyParticipantAway(tournament, participant, match, options = {}) {
  if (!tournament || !participant || participant.forfeitedAt || match?.winner) return false;
  if (match) {
    const entry = friendlyParticipantEntry(participant.id);
  ...
  participant.disconnectScore = String(options.score || participant.disconnectScore || match?.liveScore || "0/0")
    ...
    .slice(0, 80);
  participant.reconnectMatchId = match?.id || null;
...
function lockFriendlyParticipantForfeit(tournament, participant, now = Date.now()) {
  if (!participant || participant.forfeitedAt) return false;
  participant.forfeitedAt = now;
  participant.graceExpiredAt = now;
  participant.awayAt = participant.awayAt || now;
  ...
  participant.reconnectGraceSeconds = null;
  participant.disconnectScore = null;
  if (tournament?.creatorParticipantId === participant.id) {
    const nextCreator = activeFriendlyParticipants(tournament)
      .filter((item) => !item.forfeitedAt && item.id !== participant.id)
      .sort((left, right) => Number(left.joinedAt || 0) - Number(right.joinedAt || 0))[0] || null;
    if (nextCreator) tournament.creatorParticipantId = nextCreator.id;
  }
  return true;
}
...
function resolveFriendlyReconnectTimeouts(tournament, now = Date.now()) {
  ...
  for (const participant of activeFriendlyParticipants(tournament)) {
    ...
    if (match?.winner) {
      changed = lockFriendlyParticipantForfeit(tournament, participant, now) || changed;
      continue;
    }
    if (match) changed = forfeitFriendlyMatchAfterDisconnect(tournament, match, participant) || changed;
    else changed = lockFriendlyParticipantForfeit(tournament, participant, now) || changed;
  }
  return changed;
}
...
function refreshFriendlyOnePointMasterSlots(tournament) {
  ...
  if (/^group[1-4]$/.test(tournament.round) && roundDone(tournament.round)) {
    ...
    tournament.masterControl = { drawRequired: false, roundKnown: true, launched: false, launchAt: null };
    return;
  }
  if (tournament.round === "group5" && roundDone("group5")) {
    ...
    const qualifiedEntries = new Set(standings.flatMap((rows) => rows.slice(0, 3).map((row) => row.entry)));
    activeFriendlyParticipants(tournament).forEach((participant) => {
      participant.eliminated = !qualifiedEntries.has(friendlyParticipantEntry(participant.id));
      if (participant.eliminated) participant.eliminatedAt ||= Date.now();
    });
  ...
  if (tournament.round === "barrage" && roundDone("barrage")) {
    for (const match of roundMatches("barrage")) {
      const loser = match.winner === match.playerA ? match.playerB : match.playerA;
      const participant = activeFriendlyParticipants(tournament).find((item) => friendlyParticipantEntry(item.id) === loser);
      if (participant) {
        participant.eliminated = true;
        participant.eliminatedAt ||= Date.now();
      }
    }
    tournament.masterBarrageWinners = roundMatches("barrage").map((match) => match.winner);
    tournament.round = "quarter";
    tournament.masterControl = { drawRequired: true, roundKnown: false, launched: false, launchAt: null };
    return;
  }
...
function resetFriendlyTournamentForNewEvent(tournament) {
  const presentParticipants = activeFriendlyParticipants(tournament)
    .filter((participant) => !participant.awayAt && !participant.forfeitedAt);
  if (!presentParticipants.length) return false;
  tournament.participants = presentParticipants;
  if (!presentParticipants.some((participant) => participant.id === tournament.creatorParticipantId)) {
    tournament.creatorParticipantId = presentParticipants
      .sort((left, right) => Number(left.joinedAt || 0) - Number(right.joinedAt || 0))[0].id;
  }
  presentParticipants.forEach((participant) => {
    ...
    participant.graceExpiredAt = null;
    participant.reconnectDeadline = null;
    participant.reconnectMatchId = null;
    participant.reconnectGraceSeconds = null;
    participant.disconnectScore = null;
  ...
  const selected = presentParticipants.filter((participant) => participant.selected);
  if (!selected.length) {
    const creator = presentParticipants.find((participant) => participant.id === tournament.creatorParticipantId);
    if (creator) creator.selected = true;
  }
  presentParticipants.filter((participant) => participant.selected)
    .slice(friendlySelectionLimit(tournament))
    .forEach((participant) => { participant.selected = false; });
  tournament.status = "waiting";
  tournament.round = "waiting";
  tournament.entries = [];
  tournament.matches = [];
  tournament.groups = { A: [], B: [] };
  tournament.seedRanks = {};
  tournament.worldRanks = {};
  tournament.ready = {};
  tournament.nextReady = {};
  tournament.champion = null;
  tournament.spectators = [];
  tournament.excludedUserIds = [];
  tournament.updatedAt = Date.now();
  return true;
}

function removeFriendlyParticipantAfterComplete(tournament, participant, now = Date.now()) {
  if (!tournament || !participant) return false;
  participant.leftAt = now;
  participant.selected = false;
  participant.awayAt = null;
  participant.reconnectDeadline = null;
...
async function handleApi(req, res) {
  ...
  if (req.method === "POST" && friendlyResumeMatch) {
    ...
    if (participant.forfeitedAt) {
      sendJson(res, 403, { error: "Votre forfait est définitif pour cet événement." });
      return;
    }
  ...
  if (req.method === "POST" && friendlySpectateMatch) {
    ...
    if (!friendlyUserCanSpectate(tournament, user)) {
      sendJson(res, 403, { error: "Cet événement privé est réservé aux joueurs validés." });
      return;
    }
  ...
  if (req.method === "POST" && friendlySettingsMatch) {
    ...
    const previousFormat = tournament.format;
    ...
    tournament.bonus = tournament.format === "onepointmaster" && previousFormat !== "onepointmaster"
      ? "reward"
      : ["none", "ascendant", "domination", "nemesis", "reward"].includes(payload.bonus) ? payload.bonus : "none";
  ...
  const friendlyNewEventMatch = url.pathname.match(/^\/api\/friendly-tournaments\/([^/]+)\/new-event$/);
  if (req.method === "POST" && friendlyNewEventMatch) {
    const payload = await readJson(req);
    const tournament = friendlyTournaments.get(friendlyNewEventMatch[1]);
    const participant = participantForToken(tournament, payload.participantId, payload.token);
    if (!tournament || !participant) {
      sendJson(res, 404, { error: "Club House introuvable." });
      return;
    }
    if (tournament.status !== "complete" || participant.id !== tournament.creatorParticipantId) {
      sendJson(res, 403, { error: "Seul le créateur peut préparer un nouvel événement terminé." });
      return;
    }
    if (!resetFriendlyTournamentForNewEvent(tournament)) {
      sendJson(res, 409, { error: "Aucun joueur présent pour relancer le Club House." });
      return;
    }
    sendJson(res, 200, { tournament: publicFriendlyTournamentInfo(req, tournament, participant) });
    return;
  ...
  const friendlyCompetitionControlMatch = url.pathname.match(/^\/api\/friendly-tournaments\/([^/]+)\/competition-control$/);
  if (req.method === "POST" && friendlyCompetitionControlMatch) {
    const payload = await readJson(req);
    const tournament = friendlyTournaments.get(friendlyCompetitionControlMatch[1]);
    const participant = participantForToken(tournament, payload.participantId, payload.token);
    ...
    if (tournament.format === "onepoint") {
      ...
      if (action === "draw") {
        if (tournament.round === "group1") {
          ...
          control.roundKnown = true;
        } else if (tournament.round === "quarter" && tournament.masterBarrageWinners?.length === 4) {
          const groupWinners = shuffleFriendlyEntries(tournament.masterGroupWinners || []);
          const barrageWinners = shuffleFriendlyEntries(tournament.masterBarrageWinners || []);
          currentRoundMatches.forEach((match, index) => {
            match.playerA = groupWinners[index] || null;
            match.playerB = barrageWinners[index] || null;
          });
          control.drawRequired = false;
          control.roundKnown = true;
        } else {
        ...
        if (control.drawRequired) {
          if (tournament.round === "group1") {
            ...
            sendJson(res, 409, { error: "Effectuez d’abord le tirage au sort." });
            return;
          }
          tournament.updatedAt = Date.now();
          sendJson(res, 200, { tournament: publicFriendlyTournamentInfo(req, tournament, participant) });
  ...
  if (req.method === "POST" && friendlyPresenceMatch) {
    ...
    const participant = participantForToken(tournament, payload.participantId, payload.token);
    if (!tournament || !participant || !["waiting", "playing", "complete"].includes(tournament.status)) {
      sendJson(res, 404, { error: "Tournoi introuvable." });
      return;
    }
    if (tournament.status === "complete") {
      if (payload.status !== "online") removeFriendlyParticipantAfterComplete(tournament, participant);
      else participant.lastSeenAt = Date.now();
      sendJson(res, 200, { ok: true, complete: true });
      return;
    }
    resolveFriendlyPresenceLosses(tournament);
    resolveFriendlyReconnectTimeouts(tournament);
  ...
  if (req.method === "POST" && friendlyLeaveMatch) {
    ...
    if (tournament.status === "complete") {
      removeFriendlyParticipantAfterComplete(tournament, participant);
      const remainingParticipants = activeFriendlyParticipants(tournament);
      if (!remainingParticipants.length) friendlyTournaments.delete(tournament.id);
      sendJson(res, 200, {
        ok: true,
    ...
    if (currentMatch) {
      ...
      graceSeconds = FRIENDLY_RECONNECT_GRACE_MS / 1000;
      participant.reconnectGraceSeconds = graceSeconds;
      participant.reconnectDeadline = now + FRIENDLY_RECONNECT_GRACE_MS;
    }
    tournament.updatedAt = Date.now();
    sendJson(res, 200, {
      ok: true,
  ...
  if (req.method === "GET" && friendlyWatchMatch) {
    ...
    if (friendlyTournamentVisibility(tournament) === "private" && !participant?.selected) {
      sendJson(res, 403, { error: "Cette rencontre privée est réservée aux joueurs validés." });
      return;
    }
  ...
  if (req.method === "POST" && friendlyResultMatch) {
    ...
    if (tournament.format === "onepoint") {
      ...
      const loserParticipant = activeFriendlyParticipants(tournament).find((item) => friendlyParticipantEntry(item.id) === loserEntry);
      if (loserParticipant) {
        loserParticipant.eliminated = true;
        loserParticipant.eliminatedAt = Date.now();
        tournament.lastEliminatedParticipantId = loserParticipant.id;
      }
    }
