...
function restoreLocalMobileMatchSession() {
  // Certains navigateurs mobiles recrÃ©ent la page sans conserver sa query string
  // aprÃ¨s une Ã©viction mÃ©moire. L'index local sert alors de filet de sÃ©curitÃ©.
...
function syncAdminDesktopViewPreference({ applyView = false } = {}) {
  ...
  if (els.adminDesktopViewSwitch) {
    ...
    els.adminDesktopViewSwitch.setAttribute(
      "aria-label",
      forced ? "Revenir Ã  la version mobile" : "Passer Ã  la version desktop",
    );
  }
...
function confrontationStatus(wins, losses) {
  ...
  if (ratio < 35) return { label: "BÃªte noire", className: "bete-noire" };
    ...
    ? sortedAiResults.map((row, index) => {
      ...
      return `<div class="profile-row confrontation-row${index >= 5 ? " profile-collapsible-item hidden" : ""}" ${index >= 5 ? 'data-profile-collapse-group="rivalries"' : ""}>
        ...
        <span class="confrontation-summary">
          ...
          <span class="confrontation-ratio" aria-label="${wins} victoires et ${losses} dÃ©faites">${wins} / ${losses}</span>
        </span>
      </div>`;
    }).join("")
...
function renderAiClubHouse() {
  ...
  if (els.aiClubHouseSummary) {
    ...
    els.aiClubHouseSummary.textContent = isMatch
      ...
      : `${AI_CLUB_HOUSE.targetSets} sets gagnants Â· ${tournamentDifficultyLabel(AI_CLUB_HOUSE.difficulty)} Â· ${bonusText} Â· ${playersText} Â· ${distributionText}`;
...
function archiveHumanMatchSession(session, status = session.status) {
  ...
  for (let limit = Math.min(20, merged.length); limit >= 1; limit -= 1) {
    try {
      ...
      // RÃ©duit progressivement l'historique si le quota local est atteint.
    }
  }
}
...
function formatPermanentBonusStats(bonus) {
  const stats = [
    ...
    Number(bonus?.precision || 0) ? `+${Number(bonus.precision)} prÃ©cision` : "",
...
async function exportHumanMatchLogsFile() {
  ...
  const payload = {
    ...
    version: GAME_VERSION,
    schemaVersion: HUMAN_MATCH_LOG_SCHEMA_VERSION,
    description: "Parties impliquant au moins un joueur humain, regroupÃ©es par match complet.",
    scope: canAccessAdminFeatures() ? "administration et navigateur local" : "joueur connectÃ©",
    summary: {
...
function newGame(options = {}) {
  ...
  if (SERVER_SYNC.enabled && SERVER_SYNC.ready && !SERVER_SYNC.isHost) {
    state.log.unshift("Seul l'hÃ´te peut relancer un Ã©change en ligne pour le moment.");
    render();
    return;
  }
  if (!preserveSet) {
  ...
  const decisiveText = state.setMatch.decisiveExchange ? " Ã‰change dÃ©cisif." : "";
  state.log = [`${playerName(state.server)} sert. L'Ã©change commence.${setText}${decisiveText}`];
...
function restoreTurnSnapshot() {
  ...
  state.pendingCoachChoice = null;
  state.pendingRemoveChoice = null;
  state.log.unshift(`${activeName} annule son tour et revient au dÃ©but de ses choix.`);
...
function showConfrontationIntro() {
  ...
  const location = state.tournament.weekly
    ? [state.tournament.competitionCity, state.tournament.competitionCountry, state.tournament.competitionFlag].filter(Boolean).join(" Â· ")
    : "";
  const roundLabel = state.tournament.active ? humanTournamentRoundLabel() || tournamentStageLabel() : state.setMatch.enabled ? "Match" : "Ã‰change";
  const backdrop = document.createElement("div");
  backdrop.className = "confrontation-intro-backdrop";
  backdrop.innerHTML = `
    <section class="confrontation-intro" role="dialog" aria-modal="true" aria-label="PrÃ©sentation de la confrontation">
      ...
      <header class="confrontation-intro-event ${location ? "has-location" : "no-location"}">
        <p class="confrontation-intro-type confrontation-sequence-item" data-confrontation-stage><span>Ã‰vÃ©nement</span><strong>${escapeHtml(eventType)}</strong><small>${escapeHtml(eventName)}</small></p>
      ...
      <div class="confrontation-countdown-shell" data-confrontation-countdown-shell aria-hidden="true">
        ...
        <button class="primary-button confrontation-start-button" type="button" data-start-confrontation>DÃ©marrer</button>
      </div>
    </section>
  `;
  document.body.append(backdrop);
...
function getCardStats(player, card, boosted) {
  ...
  if (!isRemise(card) && playerHasSurfaceBonus(player, "grassPowerVolleySmash") && ["VolÃ©e", "Smash"].includes(card.family)) surfacePowerBonus += 2;
...
function satisfiesReturnServiceRestriction(card) {
  ...
  return !restrictionActive || !["VolÃ©e", "Smash"].includes(card.family);
}

const COLOR_BOOST_RULES = {
  Passing: ["Smash", "Amortie", "VolÃ©e"],
  VolÃ©e: ["Passing"],
