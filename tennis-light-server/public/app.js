Warning: truncated output (original token count: 277449)
... 61219 bytes omitted ...
...
function confrontationStatus(sequence, humanLevel = 1) {
  ...
  if (wins === 0) return { label: "Bête noire IA", className: "bete-noire", target: "ai", bonusCount: advancedHuman ? 3 : 1 };
  return null;
}
    ...
    ? sortedAiResults.map((row, index) => {
      ...
      return `<div class="profile-row confrontation-row${index >= 5 ? " profile-collapsible-item hidden" : ""}" ${index >= 5 ? 'data-profile-collapse-group="rivalries"' : ""}>
        ...
        <span class="confrontation-summary">
          ...
          <span class="confrontation-trend" aria-label="Série récente ${escapeHtml(trend)}">${confrontationTrendMarkup(trend)}</span>
          <span class="confrontation-ratio" aria-label="${trendWins} victoires et ${trendLosses} défaites sur les derniers matchs">${trendWins}/${trendLosses}</span>
        </span>
      </div>`;
    }).join("")
...
function renderAiClubHouse() {
  ...
  if (els.aiClubHouseSummary) {
    ...
    els.aiClubHouseSummary.textContent = isMatch
      ...
      : `${AI_CLUB_HOUSE.tournamentSize} joueurs · ${AI_CLUB_HOUSE.targetSets} sets gagnants · ${tournamentDifficultyLabel(AI_CLUB_HOUSE.difficulty)} · ${bonusText} · ${playersText} · ${distributionText}`;
...
function buildUltimateDeck(playerIndex) {
  ...
  const playerLabel = config.key === "conti" ? "Conti" : "Brentwood";
  const auditedShots = (window.ULTIMATE_CARD_DATA || []).filter((card) => card.player === playerLabel && card.type === "COUP");
  ...
  const officialShots = Array.from({ length: 36 }, (_, index) => {
    ...
    if (audited) {
      card.name = audited.name;
      card.family = audited.name.includes("SERVICE") ? "Service"
        : audited.name.includes("REVERS") ? "Revers"
          : audited.name.includes("VOLEE") ? "Volée"
            : audited.name.includes("SMASH") ? "Smash"
              : audited.name.includes("LOB") ? "Lob"
                : audited.name.includes("AMORTIE") ? "Amortie"
                  : audited.name.includes("PASSING") ? "Passing"
                    : "Coup droit";
      card.cost = audited.cost;
      card.power = audited.power;
      card.precision = audited.precision;
      card.placement = audited.placement;
      card.boostPower = audited.boostPower;
      card.boostPrecision = audited.boostPrecision;
      card.effect = audited.effect;
      card.boostCondition = audited.boostCondition;
      card.effectType = null;
      if (/Au SERVICE, cette frappe peut être BOOSTÉE/i.test(audited.effect)) card.effectType = "serviceCard";
      else if (/Peut être jouée en BOOST après un SERVICE non BOOSTÉ/i.test(audited.effect)) card.effectType = "serviceBoostHint";
      else if (/Piochez 2 cartes/i.test(audited.effect)) { card.effectType = "drawCard"; card.effectValue = 2; }
      else if (/Piochez (une|1) carte/i.test(audited.effect) && !/par BOOST adverse repoussé/i.test(audited.effect)) { card.effectType = "drawCard"; card.effectValue = 1; }
      else if (/Annulez l’effet de la prochaine carte adverse/i.test(audited.effect)) card.effectType = "cancelOpponentNextEffect";
      else if (/prochain coup gagne \+2 précision/i.test(audited.effect)) { card.effectType = "nextPrecision"; card.effectValue = 2; }
      else if (/prochain coup gagne \+2 placement/i.test(audited.effect)) { card.effectType = "nextPlacement"; card.effectValue = 2; }
      else if (/Côté faible.*limité à REVERS, LOB ou REMISE/i.test(audited.effect)) { card.effectType = "limitOpponentFamilies"; card.effectFamilies = ["Revers", "Lob", "Remise"]; }
      else if (/Votre prochain coup coûte 1 endurance de moins/i.test(audited.effect)) { card.effectType = "nextDiscount"; card.effectValue = 1; }
      else if (/Le prochain COUP adverse coûte \+1 endurance/i.test(audited.effect)) { card.effectType = "ultimateOpponentNextExtraCost"; card.effectValue = 1; }
      else if (/Chaque autre carte COUP visible.*\+1 puissance/i.test(audited.effect)) card.effectType = "ultimateOtherShotsEndPower";
      else if (/À partir du 3e coup adverse.*-2 précision/i.test(audited.effect)) card.effectType = "ultimateOpponentPrecisionAfterThird";
      else if (/Si vous gagnez, commencez l’échange suivant avec \+1 endurance/i.test(audited.effect)) card.effectType = "ultimateWinnerNextEndurance";
      else if (/Chaque carte BOOSTÉE.*\+2 puissance/i.test(audited.effect)) { card.effectType = "boostedBonusAtEnd"; card.effectValue = 2; }
      else if (/ne peut pas jouer de carte de sa réserve au prochain coup/i.test(audited.effect)) card.effectType = "ultimateLockReserveNext";
      else if (/ne peut plus utiliser sa réserve pendant cet échange/i.test(audited.effect)) card.effectType = "ultimateLockReserveExchange";
      else if (/ne peut engager qu’une seule carte EFFET/i.test(audited.effect)) card.effectType = "ultimateLimitOpponentEffect";
      else if (/ne peut plus rejouer une famille de COUP/i.test(audited.effect)) card.effectType = "ultimateNoRepeatedFamily";
      else if (/(prochain COUP.*puissance.*1|COUP adverse ne vaut que 1 puissance)/i.test(audited.effect)) card.effectType = "ultimateOpponentPowerOneIfMiss";
      else if (/cartes adverses suivantes valent au maximum 3 puissance/i.test(audited.effect)) card.effectType = "ultimateOpponentPowerCapThree";
      else if (/Doublez la puissance du premier COUP/i.test(audited.effect)) card.effectType = "ultimateDoubleFirstShot";
      else if (/Doublez en fin d’échange la carte la plus puissante/i.test(audited.effect)) card.effectType = "ultimateDoubleStrongestAfter";
      else if (/chaque nouveau COUP.*précision croissante/i.test(audited.effect)) card.effectType = "ultimateGrowingPrecision";
      else if (/Récupérez 1 endurance par énergie/i.test(audited.effect)) card.effectType = "ultimateEndurancePerEnergy";
      else if (/Choisissez une carte dans votre pioche/i.test(audited.effect)) card.effectType = "recoverUndealt";
      else if (/BOOST adverses coûtent \+1 endurance/i.test(audited.effect)) card.effectType = "ultimateOpponentBoostExtraCost";
      else if (/Après 3 coups identiques, effectuez une DRAFT/i.test(audited.effect)) card.effectType = "ultimateDraftAfterThreeFamily";
      else if (/Après 3 coups identiques, gagnez \+4 puissance/i.test(audited.effect)) card.effectType = "ultimatePowerAfterThreeFamily";
      else if (/Vous pourrez tenter un BOOST sur votre prochain coup/i.test(audited.effect)) card.effectType = "ultimateFreeBoostNext";
      else if (/Peut être BOOSTÉE si votre endurance est inférieure/i.test(audited.effect)) card.effectType = "ultimateLowEnduranceBoost";
      else if (/Récupérez une carte dans votre défausse/i.test(audited.effect)) card.effectType = "ultimateRecoverDiscard";
      else if (/Empêche l’adversaire de marquer vos cartes/i.test(audited.effect)) card.effectType = "ultimateProtectDiscardMarks";
      else if (/Placez un marqueur défausse sur une carte adverse/i.test(audited.effect)) card.effectType = "ultimateMarkOpponentOne";
      else if (/Placez deux marqueurs défausse/i.test(audited.effect)) card.effectType = "ultimateMarkOpponentTwo";
      else if (/Marquez deux cartes adverses.*ou défaussez une carte de sa réserve/i.test(audited.effect)) card.effectType = "ultimateMarkTwoOrDiscardReserve";
      else if (/Piochez 1 carte par BOOST adverse repoussé/i.test(audited.effect)) card.effectType = "ultimateDrawPerDefendedBoost";
      else if (/BOOSTER avec une carte provenant de votre défausse/i.test(audited.effect)) card.effectType = "ultimateBoostFromDiscard";
...
function ultimateReserveCandidates(playerIndex) {
  const player = state.players[playerIndex];
  if (!player) return [];
  return (player.played || []).filter((card) => !isRemise(card) && !card._fromReserve && !card.markedForDiscard && !card.removed);
}
...
function resolveUltimateReserveChoice(cardUid) {
  ...
  if (state.players[0].reserve.length > 2) {
...
function openUltimateReserveTrimChoice() {
  const player = state.players[0];
  const dialog = document.createElement("section");
  dialog.className = "ultimate-dialog";
  dialog.innerHTML = `<div class="ultimate-dialog-card"><p class="eyebrow">Réserve · maximum 2 cartes</p><h2>Choisissez la carte à supprimer</h2><p>Vous avez 3 cartes prévues pour la réserve. Une carte doit rejoindre la défausse.</p><div class="ultimate-draft-cards">${player.reserve.map((card) => `<button class="ultimate-draft-card" type="button" data-trim-reserve="${escapeHtml(card.uid)}"><img src="${escapeHtml(cardArtwork(card))}" alt="${escapeHtml(card.name)}"><strong>${escapeHtml(card.name)}</strong></button>`).join("")}</div></div>`;
  dialog.querySelectorAll("[data-trim-reserve]").forEach((button) => button.addEventListener("click", () => {
    const removed = player.reserve.find((card) => card.uid === button.dataset.trimReserve);
    player.reserve = player.reserve.filter((card) => card.uid !== button.dataset.trimReserve);
    if (removed) state.ultimateDiscards[0].push(removed);
    dialog.remove();
    continueUltimatePostExchangeDistribution();
  }));
...
function openUltimateEnergyChoice(playerIndex) {
  const player = state.players[playerIndex];
  ...
  dialog.querySelectorAll("[data-energy-choice]").forEach((button) => button.addEventListener("click", () => {
    ...
    close();
    spendUltimateEnergy(playerIndex, action, true);
  }));
...
function confirmUltimatePlayer(characterIndex) {
  ...
  SOLO_AI.enabled = true;
  SOLO_AI.playerIndex = 1;
  SOLO_AI.difficulty = ULTIMATE_MODE.aiDifficulty;
  SOLO_AI.style = ULTIMATE_MODE.aiDifficulty;
  state.ultimateDecks = [[], []];
  state.ultimateDiscards = [[], []];
  resetSetMatch();
...
function newGame(options = {}) {
  ...
  const previousUltimatePlayers = ULTIMATE_MODE.active && preserveSet && !resetUltimate && state.players.length === 2
    ? state.players.map((player) => ({
      ...
      ultimateNextExchangeEndurance: Number(player.ultimateNextExchangeEndurance || 0),
    }))
    : null;
  ...
  if (previousUltimatePlayers) {
    previousUltimatePlayers.forEach((previous, playerIndex) => {
      ...
      player.endurance = STARTING_ENDURANCE + previous.ultimateNextExchangeEndurance;
      player.ultimateNextExchangeEndurance = 0;
    });
  }
  if (SERVER_SYNC.enabled) {
...
function currentOpponentConfrontationStatus() {
  if (!state.tournament.active || state.tournament.difficulty !== "circuit" || !SOLO_AI.enabled) return null;
  ...
  if (!row) return null;
  const trend = confrontationTrend(row);
  const status = confrontationStatus(trend, circuitHumanLevel());
  return status ? { ...status, trend } : null;
}
...
function showConfrontationIntro() {
  ...
  const rivalry = currentOpponentConfrontationStatus();
  const rivalryWins = rivalry ? [...rivalry.trend].filter((result) => result === "V").length : 0;
  const backdrop = document.createElement("div");
  ...
  backdrop.innerHTML = `
    <section class="confrontation-intro" role="dialog" aria-modal="true" aria-label="Présentation de la confrontation">
      ...
      ${rivalry ? `<div class="confrontation-intro-rivalry ${rivalry.className} confrontation-sequence-item" data-confrontation-stage>
        <span>${escapeHtml(rivalry.label)}</span>
        <span class="confrontation-trend">${confrontationTrendMarkup(rivalry.trend)}</span>
        <strong>${rivalryWins}/${5 - rivalryWins}</strong>
      </div>` : ""}
...
function getCardStats(player, card, boosted) {
  ...
  if (!isRemise(card) && player.ultimateOpponentPrecisionAfterThird && player.played.filter((played) => isShot(played) && !played.removed).length >= 2) precision -= 2;
  if (!isRemise(card) && Number.isInteger(player.ultimateGrowingPrecisionStart)) {
    precision += Math.max(1, player.played.filter((played) => isShot(played) && !played.removed).length - player.ultimateGrowingPrecisionStart + 1);
  }
...
function canPlayBoost(playerIndex, card) {
  ...
  const hasSacrifice = player.hand.some((candidate) => candidate.uid !== card.uid)
    ...
    || (ULTIMATE_MODE.active && player.ultimateBoostFromDiscard && (state.ultimateDiscards[playerIndex] || []).length > 0);
...
function maybeSpendUltimateEnergyForAI(playerIndex) {
...
function registerSoloTurnRecovery(playerIn…12152 tokens truncated…ddNextPlacementBonus(player, 2, playedCard.playedUid);
...
function openUltimateCharacterState(playerIndex, powerJustActivated = false) {
  if (!ULTIMATE_MODE.active) return;
  const player = state.players[playerIndex];
  const config = ultimatePlayerConfig(playerIndex);
  if (!player || !config) return;
  const powered = Boolean(player.characterStarActive);
  const dialog = document.createElement("section");
  dialog.className = "ultimate-dialog";
...
function playCard(playerIndex, cardUid, boosted = false, sacrificeUid = null, remiseMode = "effect") {
  ...
  if (boosted) {
    if (player.hand.some((item) => item.uid === sacrificeUid)) removeFromHand(player, sacrificeUid);
      ...
      state.ultimateDiscards[playerIndex] = state.ultimateDiscards[playerIndex].filter((item) => item.uid !== sacrificeUid);
    }
  }
  ...
  if (ULTIMATE_MODE.active && answeredBoostConstraint && player.ultimateDrawPerDefendedBoost && !placementWasInsufficient) {
    const drawn = drawCards(player, 1);
    if (drawn) state.log.unshift(`${displayPlayerName(player)} pioche 1 carte après avoir repoussé un BOOST adverse.`);
  }
  if (ULTIMATE_MODE.active && player.ultimateDraftAfterThreeFamily && isShot(playedCard)) {
    const sameFamilyCount = player.played.filter((candidate) => isShot(candidate) && !candidate.removed && candidate.family === playedCard.family).length;
    if (sameFamilyCount >= 3) {
      player.ultimateDraftAfterThreeFamily = false;
      if (playerIndex === 0) window.setTimeout(() => beginUltimateDraft(playerIndex, 0, "energy"), 0);
      else {
        const choices = ultimateDrawThree(playerIndex);
        const ranked = [...choices].sort((left, right) => soloCardScore(playerIndex, right) - soloCardScore(playerIndex, left));
        player.hand.push(...ranked.slice(0, 2));
      ...
      state.log.unshift(`${displayPlayerName(player)} déclenche une DRAFT après trois COUPS de la même famille.`);
    }
  }
  ...
  if (placementWasInsufficient && player.ultimateInstantPlacementLossTo != null) {
    const winnerIndex = player.ultimateInstantPlacementLossTo;
    player.ultimateInstantPlacementLossTo = null;
    finishGame({ forcedWinner: winnerIndex, winType: "effect", reason: `${displayPlayerName(player)} ne répond pas avec assez de placement : ${displayPlayerName(state.players[winnerIndex])} gagne immédiatement l’échange grâce à son pouvoir.` });
    return;
  }
...
function completePlayedCardResolution(playerIndex, opponentIndex, card, playedCard, isOpeningServe, placementWasInsufficient, boosted, remiseMode = "effect", answeredBoostConstraint = false) {
  ...
  player.ultimateReserveLockedNext = false;
...
function applyEffect(playerIndex, card) {
  ...
  switch (card.effectType) {
    ...
    case "nextDiscount":
      ...
      setEffectNotice("appliqué", card, `La réduction est déjà incluse dans le coût affiché sur chaque carte COUP jouable.`);
      break;
    ...
    case "limitOpponentFamilies":
      ...
      setEffectNotice("appliqué", card, `${displayPlayerName(opponent)} est limité à Revers, Lob ou Remise sur son prochain coup.`);
      break;
    case "ultimateOpponentNextExtraCost":
      addNextExtraCost(opponent, card.effectValue || 1, card.playedUid);
      state.log.unshift(`Le prochain coup de ${displayPlayerName(opponent)} coûte ${card.effectValue || 1} endurance de plus.`);
      setEffectNotice("appliqué", card, `Le surcoût est déjà inclus dans les coûts affichés à ${displayPlayerName(opponent)}.`);
      break;
    case "ultimateOtherShotsEndPower":
      player.endBonuses.push({ type: "ultimateOtherShots", sourceUid: card.playedUid });
      state.log.unshift(`${displayPlayerName(player)} gagnera +1 par autre COUP visible en fin d’échange.`);
      break;
    case "ultimateOpponentPrecisionAfterThird":
      opponent.ultimateOpponentPrecisionAfterThird = true;
      state.log.unshift(`À partir de son 3e COUP, ${displayPlayerName(opponent)} subira -2 précision.`);
      break;
    case "ultimateWinnerNextEndurance":
      player.endBonuses.push({ type: "ultimateWinnerNextEndurance", sourceUid: card.playedUid });
      break;
    case "ultimateLockReserveNext":
      opponent.ultimateReserveLockedNext = true;
      setEffectNotice("appliqué", card, `${displayPlayerName(opponent)} ne peut pas jouer sa réserve sur son prochain coup.`);
      break;
    case "ultimateLockReserveExchange":
      opponent.ultimateReserveLockedExchange = true;
      setEffectNotice("appliqué", card, `${displayPlayerName(opponent)} ne peut plus utiliser sa réserve pendant cet échange.`);
      break;
    case "ultimateLimitOpponentEffect":
      opponent.ultimateEffectLimit = true;
      setEffectNotice("appliqué", card, `${displayPlayerName(opponent)} ne peut engager qu’une seule carte EFFET pendant cet échange.`);
      break;
    case "ultimateNoRepeatedFamily":
      opponent.ultimateNoRepeatedFamily = true;
    ...
    case "ultimateOpponentPowerCapThree":
      ...
      setEffectNotice("appliqué", card, `Les prochains COUPS de ${displayPlayerName(opponent)} sont limités à 3 puissance, BOOST compris.`);
      break;
    case "ultimateDoubleFirstShot":
      player.endBonuses.push({ type: "ultimateDoubleFirstShot", sourceUid: card.playedUid });
      break;
    case "ultimateDoubleStrongestAfter":
      player.endBonuses.push({ type: "ultimateDoubleStrongestAfter", sourceUid: card.playedUid });
      break;
    case "ultimateGrowingPrecision":
      player.ultimateGrowingPrecisionStart = player.played.filter((played) => isShot(played) && !played.removed).length;
      break;
    case "ultimateEndurancePerEnergy": {
      const gained = Number(player.energy || 0);
      player.endurance += gained;
      state.log.unshift(`${displayPlayerName(player)} récupère ${gained} endurance grâce à ses ${player.energy} énergies disponibles.`);
      break;
    }
    case "ultimateOpponentBoostExtraCost":
      opponent.ultimateBoostExtraCost = true;
      setEffectNotice("appliqué", card, `Chaque BOOST de ${displayPlayerName(opponent)} coûte +1 endurance pendant cet échange.`);
      break;
    case "ultimateDraftAfterThreeFamily":
      player.ultimateDraftAfterThreeFamily = true;
      break;
    case "ultimatePowerAfterThreeFamily":
      player.endBonuses.push({ type: "ultimatePowerAfterThreeFamily", sourceUid: card.playedUid });
      break;
    case "ultimateFreeBoostNext":
      player.freeBoostNext = true;
      player.freeBoostNextSourceUid = card.playedUid;
      break;
    case "ultimateRecoverDiscard": {
      if (playerIndex === 0) openUltimateCardRecoveryChoice(playerIndex, "discard");
      else {
        const recovered = state.ultimateDiscards[playerIndex].pop();
        if (recovered) player.hand.push(recovered);
      }
      break;
    }
    case "ultimateProtectDiscardMarks":
      player.ultimateDiscardMarkProtection = true;
      break;
    case "ultimateMarkOpponentOne":
    case "ultimateMarkOpponentTwo":
    case "ultimateMarkTwoOrDiscardReserve": {
      if (opponent.ultimateDiscardMarkProtection) {
    ...
    case "ultimateDrawPerDefendedBoost":
      player.ultimateDrawPerDefendedBoost = true;
      break;
    case "ultimateBoostFromDiscard":
      player.ultimateBoostFromDiscard = true;
      break;
    case "ultimateWeakestCopiesStrongest":
      player.endBonuses.push({ type: "ultimateWeakestCopiesStrongest", sourceUid: card.playedUid });
      break;
    case "ultimateInstantPlacementWin":
      opponent.ultimateInstantPlacementLossTo = playerIndex;
      break;
    ...
    case "ultimateHandForEndurance": {
      ...
      if (playerIndex === 0 && canUseSeat(playerIndex) && player.hand.length) {
        openUltimateHandDiscardChoice(playerIndex, Math.min(2, player.hand.length), gained);
      } else {
    ...
    case "ultimateRecoverReserve":
      ...
      break;
    case "recoverUndealt": {
      if (playerIndex === 0) openUltimateCardRecoveryChoice(playerIndex, "deck");
      else {
        const deck = state.ultimateDecks[playerIndex] || [];
        const selected = [...deck].sort((left, right) => soloCardScore(playerIndex, right) - soloCardScore(playerIndex, left))[0];
        if (selected) player.hand.push(selected);
        state.ultimateDecks[playerIndex] = shuffle(deck.filter((card) => card.uid !== selected?.uid));
      }
      state.log.unshift(`${displayPlayerName(player)} choisit une carte puis mélange sa pioche.`);
      break;
    }
...
function finishGame({ forcedWinner = null, ignoreScore = false, winType = "power", reason, extraPowerDetails = [] }) {
  ...
  state.players.forEach((player, playerIndex) => {
    if (playerIndex === winner && player.endBonuses.some((bonus) => bonus.type === "ultimateWinnerNextEndurance")) {
      player.ultimateNextExchangeEndurance = 1;
      state.log.unshift(`${displayPlayerName(player)} commencera le prochain échange avec +1 endurance.`);
    }
  });
  ...
  if (state.setMatch.enabled) {
    ...
    advanceProgressiveTournamentScores();
  }
...
function buildChampionshipSetup(humanCharacterId) {
  ...
  const seededDraw = shuffle(seeds);
  const others = shuffle(ranked.slice(8));
  const groups = {};
  "ABCDEFGH".split("").forEach((group, index) => {
    groups[group] = [seededDraw[index], others[index * 2], others[index * 2 + 1]];
  });
...
function startChampionshipMode(targetSets = 2, options = {}) {
  ...
  state.tournament = {
    ...
    championshipDrawOrder: [
      ..."ABCDEFGH".split("").map((group) => setup.groups[group][0]),
      ..."ABCDEFGH".split("").map((group) => setup.groups[group][1]),
      ..."ABCDEFGH".split("").map((group) => setup.groups[group][2]),
    ],
...
function handleChampionshipMatchComplete() {
  ...
  if (match.id === "final") {
    ...
    state.tournament.currentMatch = null;
    state.tournament.nextHumanMatchId = null;
    state.tournament.championCharacterId = match.winner;
    render();
    return;
  }
  if (!championshipHumanStillQualified() || (match.championshipPhase >= 3 && match.winner !== humanTournamentEntry())) {
    state.tournament.championshipHumanEliminated = true;
    state.tournament.stage = "championshipLobby";
    state.tournament.currentMatch = null;
    state.tournament.nextHumanMatchId = null;
    render();
  ...
  state.tournament.nextHumanMatchId = next?.id || null;
  render();
}
...
function tournamentBaseEntry(entry) {
  return String(entry || "").split("::duplicate:")[0];
}
...
function seededTournamentSlotMap(size, seedCount) {
  ...
  const slots32 = { 1: 1, 8: 8, 5: 9, 4: 16, 3: 17, 6: 24, 7: 25, 2: 32 };
  return Object.fromEntries(Object.entries(slots32).map(([seed, slot]) => [seed, Math.max(1, Math.round(slot * factor))]));
}
...
function buildAiClubHouseClassicSetup(options = {}) {
  ...
  const seedCount = size === 8 ? 4 : 8;
  const seedEntries = rankedRoster.slice(0, seedCount);
  if (options.distribution === "ranking") {
    const slots = seededTournamentSlotMap(size, seedCount);
    seedEntries.forEach((entry, index) => { positions[slots[index + 1]] = entry; });
  ...
  return {
    ...
    seededHistorics: [],
    seedEntries,
    seedNumbers: options.distribution === "ranking"
      ? Object.fromEntries(seedEntries.map((entry, index) => [entry, index + 1]))
      : {},
...
function startTournamentMode(targetSets = 2, options = {}) {
  ...
  const tournamentSetup = aiClubHouse
    ? buildAiClubHouseClassicSetup({
      ...
      tournamentSize: options.tournamentSize || 16,
    })
  ...
  state.tournament = {
    ...
    matchBonusDetails: [],
    pointsRecorded: false,
    stage: positions.length - 1 === 32 ? "round32" : positions.length - 1 === 16 ? "round16" : "quarter",
    targetSets,
    humanCharacterId,
    humanNickname: nicknameValue(),
    humanEntry: HUMAN_TOURNAMENT_ENTRY,
  ...
  state.log.unshift(`${tournamentLabel}${surfaceText} : ${firstHumanMatch?.label || "Premier tour"} contre ${characterNameFromId(SOLO_AI.characterId)}${onePointGame ? " · un seul point décisif" : ""}.`);
  render();
}
...
function tournamentRankIa(entry) {
  entry = tournamentBaseEntry(entry);
...
function tournamentHeadToHeadBonus(aiCharacterId) {
  if (state.tournament?.difficulty !== "circuit" || COACH_OPTIONS.includes(aiCharacterId)) return null;
  const profile = AUTH_STATE.profileUserId === authenticatedUserId() ? AUTH_STATE.profile : null;
  const row = (profile?.aiResults || []).find((result) => (
...
function applyHumanAscendantBonus() {
  if (!SOLO_AI.enabled || SERVER_SYNC.enabled || !state.tournament.active || state.tournament.difficulty !== "circuit") return;
  if (state.tournament.onePointGame && state.tournament.bonusLevel === "reward") return;
  const headToHead = tournamentHeadToHeadBonus(state.players[1].characterId);
  const targetIndex = headToHead?.target === "ai" ? 1 : 0;
  const momentum = state.setMatch.momentum?.[targetIndex];
  if (!momentum || momentum.ascendantResolved) return;
  momentum.ascendantResolved = true;
  const bonusCount = Number(headToHead?.bonusCount || 0);
  for (let index = 0; index < bonusCount; index += 1) {
    const target = state.players[targetIndex];
    const bonus = wrappedRandomBonus(
      target,
      headToHead.label,
      `${headToHead.label} · ${headToHead.trend}`,
    );
    persistMatchPermanentAward(targetIndex, bonus);
  }
}
...
function buildWeeklyTournamentMatches(positions, humanEntry, targetSets) {
  ...
  const size = positions.length - 1;
  const rounds = size === 32
    ? [{ key: "round32", prefix: "r32", label: "16e de finale", count: 16 }, { key: "round16", prefix: "r16", label: "8e de finale", count: 8 }]
    : size === 16
      ? [{ key: "round16", prefix: "r16", label: "8e de finale", count: 8 }]
      : [];
  rounds.push(
    { key: "quarter", prefix: "qf", label: "Quart de finale", count: 4 },
    { key: "semi", prefix: "semi", label: "Demi-finale", count: 2 },
    { key: "final", prefix: "final", label: "Finale", count: 1 },
  );
  const matches = [];
  ...
  rounds.forEach((round, roundIndex) => {
    for (let index = 0; index < round.count; index += 1) {
      ...
      const firstRoundPair = state.tournament?.weekly && size === 16
        ? circuitPositionPairs[index]
        : [(index * 2) + 1, (index * 2) + 2];
      const id = round.key === "final" ? "final" : `${round.prefix}${round.prefix.startsWith("r") ? "_" : ""}${index + 1}`;
      matches.push(match(
        id,
        round.count === 1 ? round.label : `${round.label} ${index + 1}`,
        round.key,
        firstRound ? positions[firstRoundPair[0]] : null,
        firstRound ? positions[firstRoundPair[1]] : null,
      ));
    }
  });
...
function aiTournamentStrength(characterId) {
  characterId = tournamentBaseEntry(characterId);
...
function progressiveTournamentScoreText(progress) {
  const completed = progress.completedScores || [];
  const current = progress.matchOver || !(progress.score || []).some(Number) ? [] : [progress.score];
  return formatSetScores([...completed, ...current]);
}

function progressiveTournamentRankRating(entry) {
  const rank = Math.max(1, Math.min(24, tournamentRankIa(entry)));
  const ratings = [
    0, -0.291, -0.438, -0.563, -0.609, -0.688, -0.766, -0.836,
    -0.906, -1.016, -1.059, -1.102, -1.178, -1.254, -1.300, -1.360,
    -1.420, -1.473, -1.530, -1.593, -1.683, -1.770, -1.852, -2.055,
  ];
  return ratings[rank - 1] ?? ratings[ratings.length - 1];
}

function progressiveTournamentHandQuality() {
...
function progressiveTournamentExchangeScore(handGap = 0) {
  const baseWeights = [0.60, 0.30, 0.10];
  const scores = [[2, 1], [2, 0], [3, 0]];
  const weights = baseWeights.map((weight, index) => weight * Math.exp(0.16 * handGap * (index - 0.65)));
...
function progressiveTournamentNextServer(progress, nextScore) {
  if (Math.max(...nextScore) === 6 && Math.min(...nextScore) === 5) {
    return nextScore[0] === 6 ? 0 : 1;
  }
...
function advanceProgressiveTournamentMatch(match, targetSetsOverride = null) {
  ...
  if (next[winner] === 7 && next[loser] < 5) next[winner] = 6;
  if (next[loser] > 6 && next[winner] < 7) next[loser] = 6;
  if (next[0] === 7 && next[1] === 7) next[loser] = 6;
  progress.momentum[winner] = Math.min(1, progress.momentum[winner] + 0.2);
  progress.momentum[loser] = Math.max(-1, progress.momentum[loser] - 0.2);
  progress.server = progressiveTournamentNextServer(progress, next);
  progress.score = next;
  if (isSetOver(next)) {
    progress.completedScores.push([...next]);
    progress.setsWon[leadingSetPlayer(next)] += 1;
    progress.score = [0, 0];
  ...
  match.revealedSetScores = progress.completedScores.map((score) => [...score]);
  if (progress.matchOver) {
    match.winner = matchWinnerIndex === 0 ? match.playerA : match.playerB;
    match.score = formatSetScores(progress.completedScores);
    match.liveScore = null;
  } else {
    match.liveScore = `${progressiveTournamentScoreText(progress)} · EN DIRECT`;
    match.score = null;
  }
  return true;
}
...
function ensureTournamentMatchHasWinningSetCount(match) {
  if (!match?.simulated || (!match.winner && !match.score)) return;
  const winner = match.winner || match.hiddenWinner;
  const winnerIndex = winner === match.playerA ? 0 : 1;
  const targetSets = Number(state.tournament.targetSets || 2);
  const scores = match.revealedSetScores?.length
    ? match.revealedSetScores
    : parseTournamentScore(match.score || "");
  const wonSets = scores.filter((score) => Number(score?.[winnerIndex] || 0) > Number(score?.[opponentOf(winnerIndex)] || 0)).length;
  if (wonSets >= targetSets && scores.length <= (targetSets * 2) - 1) return;
  const repaired = randomMatchSetScoresForWinner(winnerIndex, targetSets);
  match.hiddenWinner = winner;
  match.hiddenSetScores = repaired.map((score) => [...score]);
  match.revealedSetScores = repaired.map((score) => [...score]);
  match.winner = winner;
  match.score = formatSetScores(repaired);
  match.liveScore = null;
}
...
function tournamentEntryCharacterId(entry) {
  ...
  return isHumanTournamentEntry(entry) ? state.tournament.humanCharacterId : tournamentBaseEntry(entry);
}

function tournamentWinnerEntryFromMatchWinner(winnerIndex) {
  const match = tournamentMatchById(state.tournament?.currentMatch);
  if (match && (match.playerA === humanTournamentEntry() || match.playerB === humanTournamentEntry())) {
    return winnerIndex === 0
      ? humanTournamentEntry()
      : (match.playerA === humanTournamentEntry() ? match.playerB : match.playerA);
  }
...
function completeWeeklyTournamentAfterHumanLoss() {
  if (state.tournament.progressiveLiveScores) {
    for (let guard = 0; guard < 8; guard += 1) {
      refreshWeeklyTournamentDerivedSlots();
      const pending = state.tournament.matches.filter((match) => match.simulated && match.playerA && match.playerB && !match.winner);
      if (!pending.length) break;
      pending.forEach((match) => {
        for (let step = 0; step < 80 && !match.winner; step += 1) advanceProgressiveTournamentMatch(match);
      });
    }
    refreshWeeklyTournamentDerivedSlots();
    const progressiveFinal = tournamentMatchById("final");
    state.tournament.stage = "complete";
    state.tournament.currentMatch = null;
    state.tournament.nextHumanMatchId = null;
    state.tournament.championCharacterId = progressiveFinal?.winner || null;
    return;
  }
  for (const match of state.tournament.matches) {
    if (match.winner || !match.playerA || !match.playerB) continue;
...
function startWeeklyNextMatch() {
  ...
  const championshipNext = state.tournament.championship ? advanceChampionshipToNextHumanMatch() : null;
  const match = championshipNext || tournamentMatchById(state.tournament.nextHumanMatchId);
  if (!match) return;
  state.tournament.stage = match.round;
  state.tournament.currentMatch = match.id;
  if (state.tournament.championship) {
...
function revealNextTournamentAiSet() {
  if (state.tournament?.progressiveLiveScores) return;
  if (state.tournament.championship) {
    const current = tournamentMatchById(state.tournament.currentMatch);
    if (!current) return;
...
function refreshWeeklyTournamentDerivedSlots() {
  ...
  if (Number(state.tournament.bracketSize || 16) !== 16) {
    const rounds = ["round32", "round16", "quarter", "semi", "final"]
      .map((round) => state.tournament.matches.filter((match) => match.round === round))
      .filter((matches) => matches.length);
    for (let roundIndex = 1; roundIndex < rounds.length; roundIndex += 1) {
      const previous = rounds[roundIndex - 1];
      rounds[roundIndex].forEach((match, index) => {
        const left = previous[index * 2];
        const right = previous[(index * 2) + 1];
        if (left?.winner || right?.winner) setMatchPlayers(match, left?.winner || null, right?.winner || null);
      });
    }
    return;
  }
...
function nextFullSet() {
  ...
  newGame({ preserveSet: true, serverOverride: server, resetUltimate: ULTIMATE_MODE.active });
  state.log.unshift("Nouveau set lancé.");
  markServerDirtyForHostAction();
  render();
  if (ULTIMATE_MODE.active) beginUltimateDraft(0, 0, "set-start");
}
...
function applyEndBonuses() {
  ...
  for (const player of state.players) {
    for (const bonus of player.endBonuses) {
      ...
      if (bonus.type === "ultimateOtherShots") {
        const count = Math.max(0, player.played.filter((card) => isShot(card) && !card.removed).length - 1);
        player.power += count;
        if (count) details.push({ playerIndex: state.players.indexOf(player), label: "Autres COUPS visibles", points: count });
      }
      if (bonus.type === "ultimateDoubleFirstShot") {
        const target = player.played.find((card) => isShot(card) && !card.removed);
        const gained = Number(target?.cardPowerGained ?? target?.powerGained ?? 0);
        player.power += gained;
        if (gained) details.push({ playerIndex: state.players.indexOf(player), label: `Premier COUP doublé · ${target.name}`, points: gained });
      }
      if (bonus.type === "ultimateDoubleStrongestAfter") {
        const sourceIndex = player.played.findIndex((card) => card.playedUid === bonus.sourceUid);
        const target = player.played.slice(sourceIndex + 1).filter((card) => isShot(card) && !card.removed).sort((a, b) => Number(b.cardPowerGained || 0) - Number(a.cardPowerGained || 0))[0];
        const gained = Number(target?.cardPowerGained ?? 0);
        player.power += gained;
        if (gained) details.push({ playerIndex: state.players.indexOf(player), label: `COUP le plus puissant doublé · ${target.name}`, points: gained });
      }
      if (bonus.type === "ultimatePowerAfterThreeFamily") {
        const counts = new Map();
        player.played.filter((card) => isShot(card) && !card.removed).forEach((card) => counts.set(card.family, (counts.get(card.family) || 0) + 1));
        if ([...counts.values()].some((count) => count >= 3)) { player.power += 4; details.push({ playerIndex: state.players.indexOf(player), label: "Trois COUPS identiques", points: 4 }); }
      }
      if (bonus.type === "ultimateWeakestCopiesStrongest") {
        const shots = player.played.filter((card) => isShot(card) && !card.boosted && !card.removed);
        const powers = shots.map((card) => Number(card.cardPowerGained || 0));
        const gained = powers.length ? Math.max(...powers) - Math.min(...powers) : 0;
        player.power += Math.max(0, gained);
        if (gained > 0) details.push({ playerIndex: state.players.indexOf(player), label: "COUP faible aligné sur le plus fort", points: gained });
      }
    }
  }
  return details;
...
function renderTournamentPanel() {
  ...
  const locationText = state.tournament.weekly
    ? [state.tournament.competitionCity, state.tournament.competitionCountry, state.tournament.competitionFlag].filter(Boolean).join(" · ")
    : "";
  const round32Matches = state.tournament.matches.filter((match) => match.round === "round32");
  ...
  els.tournamentPanel.innerHTML = `
    ...
    <div class="tournament-bracket ${TOURNAMENT_PANEL_UI.visible ? "" : "hidden"}">
      ${round32Matches.length ? `
        <div class="tournament-column round32-column">
          <span class="tournament-round-label tournament-column-title">16es</span>
          ${round32Matches.map((match) => renderTournamentMatch(match)).join("")}
        </div>
      ` : ""}
...
function championshipDrawnEntrySet() {
  return new Set((state.tournament.championshipDrawOrder || []).slice(0, Number(state.tournament.championshipDrawVisibleCount || 0)));
}

function championshipLobbyPlayer(entry, drawn = championshipDrawnEntrySet()) {
...
function championshipNextPendingBatch() {
...
function renderChampionshipLobby() {
  ...
  if (state.tournament.championshipDrawComplete == null) {
    state.tournament.championshipDrawOrder = state.tournament.championshipDrawOrder
      || [
        ..."ABCDEFGH".split("").map((group) => state.tournament.championshipPhase1Groups?.[group]?.[0]).filter(Boolean),
        ..."ABCDEFGH".split("").map((group) => state.tournament.championshipPhase1Groups?.[group]?.[1]).filter(Boolean),
        ..."ABCDEFGH".split("").map((group) => state.tournament.championshipPhase1Groups?.[group]?.[2]).filter(Boolean),
      ];
    completeChampionshipDraw();
  }
  ...
  const finalDrawPending = Boolean(
    state.tournament.championshipFinalDraw?.length
    && Number(state.tournament.championshipFinalDrawVisibleCount || 0) < state.tournament.championshipFinalDraw.length
  );
...
function completeChampionshipDraw() {
  state.tournament.championshipDrawVisibleCount = state.tournament.championshipDrawOrder.length;
  state.tournament.championshipDrawComplete = true;
}

function startChampionshipDraw(animated = true) {
  if (CHAMPIONSHIP_LOBBY_UI.busy) return;
  const finalDraw = state.tournament.championshipDrawComplete && state.tournament.championshipFinalDraw?.length
    ? state.tournament.championshipFinalDraw
    : null;
  if (state.tournament.championshipDrawComplete && !finalDraw) return;
  if (finalDraw && Number(state.tournament.championshipFinalDrawVisibleCount || 0) >= finalDraw.length) return;
  window.clearInterval(CHAMPIONSHIP_LOBBY_UI.timer);
  if (!animated) {
    if (finalDraw) state.tournament.championshipFinalDrawVisibleCount = finalDraw.length;
    else completeChampionshipDraw();
    renderChampionshipLobby();
    return;
  }
  CHAMPIONSHIP_LOBBY_UI.busy = true;
  renderChampionshipLobby();
  CHAMPIONSHIP_LOBBY_UI.timer = window.setInterval(() => {
    if (finalDraw) state.tournament.championshipFinalDrawVisibleCount += 1;
    else state.tournament.championshipDrawVisibleCount += 1;
    const done = finalDraw
      ? state.tournament.championshipFinalDrawVisibleCount >= finalDraw.length
      : state.tournament.championshipDrawVisibleCount >= state.tournament.championshipDrawOrder.length;
    if (done) {
      window.clearInterval(CHAMPIONSHIP_LOBBY_UI.timer);
      if (finalDraw) state.tournament.championshipFinalDrawVisibleCount = finalDraw.length;
      else completeChampionshipDraw();
      CHAMPIONSHIP_LOBBY_UI.busy = false;
    }
    renderChampionshipLobby();
  }, 1000);
}

function simulateChampionshipBatchAnimated() {
  if (CHAMPIONSHIP_LOBBY_UI.busy || !state.tournament.championshipDrawComplete) return;
  const pending = championshipNextPendingBatch();
  if (!pending) return;
  const matches = championshipMatches(pending.championshipPhase, pending.day).filter((match) => !match.winner);
  if (matches.some((match) => isHumanTournamentEntry(match.playerA) || isHumanTournamentEntry(match.playerB))) return;
  CHAMPIONSHIP_LOBBY_UI.busy = true;
  let index = 0;
  const showNext = () => {
    ...
    if (ensureSimulatedTournamentMatchReady(match)) {
      match.revealedSetScores = match.hiddenSetScores.map((score) => [...score]);
      match.score = formatSetScores(match.revealedSetScores);
      match.winner = match.hiddenWinner;
      match.liveScore = null;
    ...
    index += 1;
    renderChampionshipLobby();
    window.setTimeout(showNext, 1000);
  };
  showNext();
}

function startChampionshipNextFromLobby() {
  if (CHAMPIONSHIP_LOBBY_UI.busy || state.tournament.stage === "complete") return;
  if (!state.tournament.championshipDrawComplete) completeChampionshipDraw();
  if (state.tournament.championshipFinalDraw?.length) {
    state.tournament.championshipFinalDrawVisibleCount = state.tournament.championshipFinalDraw.length;
  }
...
function tournamentPlayerLabel(entry) {
  ...
  return isHumanTournamentEntry(entry)
    ...
    : characterNameFromId(tournamentBaseEntry(entry));
}
...
function renderCenterNextSetButton() {
  ...
  if (state.tournament.active && state.gameOver && state.setMatch.matchOver) {
    ...
    if (state.tournament.stage === "readyNext") {
      return state.tournament.championship
        ? '<button class="primary-button next-exchange-button next-set-button" type="button" data-start-tournament-next-match>MATCH SUIVANT</button><button class="small-button next-set-button" type="button" data-return-championship-lobby>Retour au Club House</button>'
        : '<button class="primary-button next-exchange-button next-set-button" type="button" data-start-tournament-next-match>MATCH SUIVANT</button>';
    }
...
function activeEffectBadges(playerIndex) {
  ...
  if (player.ultimateEffectLimit) badges.push({ text: "Pendant l’échange : 1 seule carte EFFET autorisée", type: "constraint", category: "permanent" });
  if (player.ultimateNoRepeatedFamily) badges.push({ text: "Pendant l’échange : impossible de rejouer une famille de COUP", type: "constraint", category: "permanent" });
  if (player.ultimateOpponentPrecisionAfterThird) badges.push({ text: "À partir du 3e COUP : -2 précision", type: "constraint", category: "permanent" });
  if (player.ultimatePowerCapThree) badges.push({ text: "Pendant l’échange : puissance des COUPS limitée à 3", type: "constraint", category: "permanent" });
  if (player.ultimateBoostExtraCost) badges.push({ text: "Pendant l’échange : chaque BOOST coûte +1 endurance", type: "constraint", category: "permanent" });
...
function renderUltimateReserveInHand(playerIndex) {
  if (!ULTIMATE_MODE.active) return "";
  const player = state.players[playerIndex];
...
function openUltimateCardRecoveryChoice(playerIndex, source) {
  const player = state.players[playerIndex];
  const cards = source === "deck" ? (state.ultimateDecks[playerIndex] || []) : (state.ultimateDiscards[playerIndex] || []);
  if (!cards.length) {
    state.log.unshift(source === "deck" ? "La pioche est vide." : "La défausse est vide.");
    return;
  }
  const dialog = document.createElement("section");
  dialog.className = "ultimate-dialog";
  ...
  dialog.querySelectorAll("[data-recover-ultimate]").forEach((button) => button.addEventListener("click", () => {
    ...
    if (source === "deck") state.ultimateDecks[playerIndex] = shuffle(state.ultimateDecks[playerIndex].filter((card) => card.uid !== selected.uid));
    else state.ultimateDiscards[playerIndex] = state.ultimateDiscards[playerIndex].filter((card) => card.uid !== selected.uid);
...
function openUltimateHandDiscardChoice(playerIndex, requiredCount, enduranceGain) {
  const player = state.players[playerIndex];
  if (!player || !requiredCount) {
    if (player) player.endurance = Math.min(STARTING_ENDURANCE, player.endurance + enduranceGain);
    return;
  }
  const selected = new Set();
  const dialog = document.createElement("section");
  ...
  const paint = () => {
    ...
    dialog.querySelectorAll("[data-discard-hand]").forEach((button) => button.addEventListener("click", () => {
      ...
      if (selected.has(uid)) selected.delete(uid);
      else if (selected.size < requiredCount) selected.add(uid);
      paint();
    }));
    dialog.querySelector("[data-confirm-hand-discard]")?.addEventListener("click", () => {
      if (selected.size !== requiredCount) return;
      const discarded = player.hand.filter((card) => selected.has(card.uid));
      player.hand = player.hand.filter((card) => !selected.has(card.uid));
      state.ultimateDiscards[playerIndex].push(...discarded);
      player.endurance = Math.min(STARTING_ENDURANCE, player.endurance + enduranceGain);
      state.log.unshift(`${displayPlayerName(player)} choisit et défausse ${discarded.length} carte(s), puis récupère ${enduranceGain} endurance.`);
      dialog.remove();
      render();
    });
...
function renderPlayerPanel(playerIndex, root) {
  ...
  root.innerHTML = `
    ...
    <div class="hand${tutorialFocusClass("hand", playerIndex)}">
      ...
      ${renderUltimateReserveInHand(playerIndex)}
    </div>
  ...
  root.querySelectorAll("[data-open-ultimate-energy]").forEach((button) => {
    button.addEventListener("click", () => openUltimateEnergyChoice(Number(button.dataset.openUltimateEnergy)));
  });
  ...
  root.querySelectorAll("[data-ultimate-character-state]").forEach((button) => {
    button.addEventListener("click", () => openUltimateCharacterState(Number(button.dataset.ultimateCharacterState)));
  });
  root.querySelectorAll("[data-use-reserve]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      player.hand.push(card);
      state.log.unshift(`${displayPlayerName(player)} joue ${card.name} depuis sa réserve : la carte reste marquée.`);
...
function renderBoostModal() {
  ...
  const discardChoices = ULTIMATE_MODE.active && player.ultimateBoostFromDiscard ? (state.ultimateDiscards[playerIndex] || []) : [];
  const choices = [...player.hand.filter((item) => item.uid !== cardUid), ...reserveChoices, ...discardChoices];
  backdrop.innerHTML = `
  ...
  backdrop.querySelectorAll("[data-sacrifice]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      const sacrificeCard = player.hand.find((item) => item.uid === sacrificeUid) || (player.reserve || []).find((item) => item.uid === sacrificeUid) || (state.ultimateDiscards[playerIndex] || []).find((item) => item.uid === sacrificeUid);
      state.pendingBoost = null;
...
function initMenu() {
  ...
  els.ultimatePlayerDialog?.querySelectorAll("[data-ultimate-ai]").forEach((button) => {
    button.addEventListener("click", () => {
      ULTIMATE_MODE.aiDifficulty = button.dataset.ultimateAi || "normal";
      els.ultimatePlayerDialog.querySelectorAll("[data-ultimate-ai]").forEach((choice) => choice.classList.toggle("selected", choice === button));
    });
  });
...
els.gameCardZoomToggle?.addEventListener("change", () => {
  GAMEPLAY_ASSIST.cardZoom = Boolean(els.gameCardZoomToggle.checked);
  localStorage.setItem("tennisLightCardZoom", String(GAMEPLAY_ASSIST.cardZoom));
  if (!GAMEPLAY_ASSIST.cardZoom) {
    closeCardLocalPreview();
    document.querySelector(".image-zoom-backdrop")?.remove();
  }
});
...
try {
  ...
  console.error("Initialisation du match neutralisée pour préserver la navigation de la home.", error);
  showMenuScreen();
}
